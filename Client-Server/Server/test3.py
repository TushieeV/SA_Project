...
c_tok = requests.get(BASE_URL + 'get-token', params={'username': 'c'})
print(c_tok.json())
...
c_send_pkey = requests.post(BASE_URL + 'my-pkey', params={'token': c_tok.json()['token'], 'pkey': str(c.generate_public_key())})
print(c_send_pkey.json())
...
c_req_b = requests.post(BASE_URL + 'request', params={'requestor': c_tok.json()['token'], 'requesting': 'b'})
print(c_req_b.json())
...
b_accept_c_req = requests.post(BASE_URL + 'accept-request', params={'req_id': c_req_b.json()['req_id'], 'token': b_tok.json()['token']})
print(b_accept_a_req.json())
...
b_get_c_pkey = requests.get(BASE_URL + 'get-pkey', params={'ses_id': b_accept_c_req.json()['ses_id'], 'target': 'c'})
print(b_get_c_pkey.json())

c_get_b_pkey = requests.get(BASE_URL + 'get-pkey', params={'ses_id': b_accept_c_req.json()['ses_id'], 'target': 'b'})
print(c_get_b_pkey.json())

a_shared = a.generate_shared_key(a_get_b_pkey.json()['pkey'])
b_shared = b.generate_shared_key(b_get_a_pkey.json()['pkey'])
cb_shared = c.generate_shared_key(c_get_b_pkey.json()['pkey'])
bc_shared = b.generate_shared_key(b_get_c_pkey.json()['pkey'])
print(a_shared)
print(b_shared)
print(cb_shared == bc_shared)
...
msg = 'Goodbye'
encrypted_msg = encrypt('Goodbye', cb_shared)
c_msg_b = requests.post(BASE_URL + 'message', params={'ses_id': b_accept_c_req.json()['ses_id'], 'sender': c_tok.json()['token'], 'receiver': 'b', 'msg': encrypted_msg, 'time': str(datetime.now())})
print(c_msg_b.json())
...
b_get_msgs = requests.get(BASE_URL + 'get-messages', params={'ses_id': b_accept_c_req.json()['ses_id']})
print(b_get_msgs.json())
decrypted_msg = decrypt(b_get_msgs.json()['messages'][0]['msg'], cb_shared)
print("Decrypted: " + decrypted_msg)
